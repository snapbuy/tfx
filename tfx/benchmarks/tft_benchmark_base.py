...
class TFTBenchmarkBase(benchmark_base.BenchmarkBase):
  ...
  def benchmarkAnalyzeAndTransformDatasetTF2(self):
  ...
  def benchmarkRunMetaGraphDoFnManualActuationTF2(self):
  ...
  def benchmarkRunMetagraphDoFnAtTFLevelTF2(self):
    ...
    inputs_tensor_keys, outputs_tensor_keys = (
        saved_model_loader.get_dependent_input_output_keys(
            saved_model_loader._structured_inputs.keys(), []))  # pylint: disable=protected-access
    ...
    for batch in batched_records:
      ...
      feed_dict = {name: feed_by_name[name] for name in inputs_tensor_keys}
      outputs_dict = callable_get_outputs(feed_dict)
      _ = {key: outputs_dict[key] for key in outputs_tensor_keys}
    end = time.time()
    delta = end - start
