...
class AbstractJob(abc.ABC):
  ...
  @abc.abstractmethod
  def get_client(self) -> discovery.Resource:
  ...
  @abc.abstractmethod
  def launch_job(self, job_spec, parent) -> None:
    """Launches a long-running job.

    Args:
      job_spec: The job specification. See
...
class CAIPJobClient(AbstractJob):
  ...
  def create_client(self,
    ...
    self._job_id = job_id  # Assigned in self.create_job_spec() if not given
    self._project_id = project_id  # Assigned in self.launch_job() if not given
    return self._client
  ...
  def create_training_args(self, input_dict: Dict[Text, List[types.Artifact]],
    ...
    self._job_id = job_id or 'tfx_{}'.format(
  ...
  def launch_job(self, job_spec: Dict[Text, Any], parent: Text) -> None:
    """Launches a long-running job.
    ...
    logging.info('TrainingInput=%s', job_spec['trainingInput'])
    logging.info('Submitting job=\'%s\', project=\'%s\' to AI Platform.',
                 job_spec['jobId'], parent)
...
def get_job_client(use_ucaip: bool = False):
  if use_ucaip:
